"""FINAL MAIN (LOSO + LOOCV + ABLATION + CSV + PLOTS)"""
...
# SPLIT FUNCTIONS
# ==========================================
def create_loso_splits(files):
    subjects = sorted(set([f.stem.split("_")[0] for f in files]))
    splits = []

    for subject in subjects:
        train = [f for f in files if subject not in f.stem]
        val   = [f for f in files if subject in f.stem]
        splits.append((subject, train, val))

    return splits


def create_loocv_splits(files):
    splits = []

    for i, f in enumerate(files):
        train = files[:i] + files[i+1:]
        val = [f]
        splits.append((f"sample_{i}", train, val))
...
# GENERIC CV RUNNER
# ==========================================
def run_cv(mode, files, args, device, plot_dir):

    print(f"\n🚀 Running mode: {mode.upper()}")

    if mode == "loso":
        splits = create_loso_splits(files)
    else:
        splits = create_loocv_splits(files)
        splits = splits[:100]  # ⚠️ limit LOOCV
    ...
    for fold_name, train_files, val_files in splits:
        ...
        plt.legend()
        plt.title(f"{mode} - {fold_name} Loss")
        plt.savefig(f"{plot_dir}/{mode}_{fold_name}_loss.png")
        plt.close()

        plt.figure()
        plt.plot(history["val_acc"])
        plt.title(f"{mode} - {fold_name} Accuracy")
        plt.savefig(f"{plot_dir}/{mode}_{fold_name}_acc.png")
        plt.close()
        ...
        print(f"{mode} fold acc: {best_acc:.3f}")
...
def run(args):
    ...
    # LOSO
    # ==========================================
    loso_results, loso_records = run_cv("loso", files, args, device, plot_dir)
    results["loso"] = loso_results
    pd.DataFrame(loso_records).to_csv("outputs/loso_results.csv", index=False)

    # ==========================================
    # LOOCV
    # ==========================================
    loocv_results, loocv_records = run_cv("loocv", files, args, device, plot_dir)
    results["loocv"] = loocv_results
    pd.DataFrame(loocv_records).to_csv("outputs/loocv_results.csv", index=False)
