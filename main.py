"""Entry point for end-to-end chewing classification experiments (RECORDED-ONLY LOSO)."""
...
def run(args):
    ...
    for tr_records, te_records, sid in loso_splits(recorded_records):
        ...
        # ==========================================
        # MODEL INIT (FRESH EACH SUBJECT)
        # ==========================================
        model = FrequencyAwareMultiTaskNet(m_cfg).to(device)

        # ❌ OLD PRETRAIN LOAD (DISABLED)
    ...
    # ✅ ADD MEAN ACCURACY (IMPORTANT FOR PAPER)
    accs = [f["accuracy"] for f in fold_tex if "accuracy" in f]

    if len(accs) > 0:
        print("Per-subject accuracy:", accs)
        print(f"Mean LOSO Accuracy: {np.mean(accs):.4f}")
...
#         # still safe even without pretraining
#         model.load_state_dict(base_model.state_dict(), strict=False)
